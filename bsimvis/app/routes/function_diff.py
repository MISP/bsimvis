...
def render_line_content(line_tokens, common_hashes, feature_map, tf_map, side="l", side_tips=None):
    tokens_json = []
    if side_tips is None: side_tips = {}
...
def render_aligned_diff(s1, f1, s2, f2, common_hashes, tf1, tf2):
    ...
    def render_side(s_data, line_idx, lines_dict, f_map, tag="", opposing_idx=None, chunk_id=0, identities=[], tf_map={}, side="l", side_tips=None):
        if line_idx is None:
            return None
        ...
        tokens_json = render_line_content(line_tokens, common_hashes, f_map, tf_map, side, side_tips)
        
        tooltip_text = f"Address: {addr} | ID: {line_identity}" if addr else f"ID: {line_identity}"
    ...
    for chunk_id, (tag, i1, i2, j1, j2) in enumerate(sm.get_opcodes()):
        if tag == 'equal' or tag == 'replace':
            ...
            for k in range(max_len):
                ...
                rows.append({
                    "l": render_side(s1, l_idx, lines1, f_map1, tag, r_idx, chunk_id, id1, tf1, "l", left_tips),
                    "r": render_side(s2, r_idx, lines2, f_map2, tag, l_idx, chunk_id, id2, tf2, "r", right_tips),
                })
        elif tag == 'insert':
            for r_idx in range(j1, j2):
                rows.append({
                    "l": render_side(s1, None, lines1, f_map1, tag, r_idx,  chunk_id, id1, tf1, "l", left_tips),
                    "r": render_side(s2, r_idx,  lines2, f_map2, tag, None, chunk_id, id2, tf2, "r", right_tips),
                })
        elif tag == 'delete':
            for l_idx in range(i1, i2):
                rows.append({
                    "l": render_side(s1, l_idx,  lines1, f_map1, tag, None, chunk_id, id1, tf1, "l", left_tips),
                    "r": render_side(s2, None, lines2, f_map2, tag, l_idx,  chunk_id, id2, tf2, "r", right_tips),
                })
