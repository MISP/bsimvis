...
def _scan_feature_keys(r, collection, feature_prefix, offset, limit, sort_by):
    ...
    if sort_by == 'tf':
        ...
        if feature_prefix:
            ...
            return [{"hash": h, "tf_score": s, "frequency": r.zcard(f"idx:{collection}:feature:{h}:functions")} for h, s in page], len(all_matches)
        else:
            total = r.zcard(zset_key)
            page = r.zrevrange(zset_key, offset, offset + limit - 1, withscores=True)
            return [{"hash": h, "tf_score": s, "frequency": r.zcard(f"idx:{collection}:feature:{h}:functions")} for h, s in page], total
    else:
