...
def similarity_search():
    ...
    try:
        ...
        if sort_by == "score" and sort_order == "desc":
            candidate_ids = r.zrevrangebyscore(
                algo_zset, max_score, threshold, start=fetch_start, num=fetch_num
            )
        elif sort_by == "score" and sort_order == "asc":
            candidate_ids = r.zrangebyscore(
                algo_zset, threshold, max_score, start=fetch_start, num=fetch_num
            )
        else:
        ...
        for sim_member in candidate_ids:
            ...
            meta_key = (
                f"{col}:sim_meta:{algo}:{parts[0]}:{parts[1]}"
                if len(parts) == 2
                else f"{col}:sim_meta:{algo}:{sim_member}"
            )
        ...
        for sim_id, raw in zip(candidate_ids, raw_results):
            ...
            if min_features > 0:
                if (
                    data.get("feat_count1", 0) < min_features
                    or data.get("feat_count2", 0) < min_features
                ):
                    continue

            if md5_filters:
                if not any(
                    m in (data.get("md5_1"), data.get("md5_2")) for m in md5_filters
                ):
                    continue
            ...
            if tag_filter and tag_filter not in t1 and tag_filter not in t2:
                continue

            l1, l2 = (
                data.get("language_id1", "").lower(),
                data.get("language_id2", "").lower(),
            )
            if lang_filter and lang_filter not in l1 and lang_filter not in l2:
                continue

            if search_q and not any(
                search_q in str(data.get(k, "")).lower()
                for k in ["name1", "name2", "id1", "id2", "tags1", "tags2"]
            ):
                continue
        ...
        if sort_by != "score" or (
            sort_by == "score" and sort_order != "desc" and sort_order != "asc"
        ):
            ...
            if sort_by == "name":
                ...
                enriched_pairs.sort(
                    key=lambda x: x["meta1"]["bsim_features_count"], reverse=reverse
                )
            elif sort_by == "score":
