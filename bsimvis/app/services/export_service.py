...
def export_to_csv(items, search_type):
    ...
    if search_type == "collections":
        ...
        headers = ["Filename", "MD5", "Language", "Batch UUID", "Functions", "Entry Date", "Tags", "User Tags"]
        writer.writerow(headers)
        for item in items:
            writer.writerow([
                item.get("file_name", ""),
        ...
        headers = [
            "Function", "Address", "Return Type", "Namespace", "Parameters",
            "Function Tags", "Function User Tags", "Cluster Names", "Cluster UUIDs",
            "Features", "File Name", "MD5", "File Tags", "File User Tags", "Language", "Date"
        ]
        writer.writerow(headers)
        for item in items:
            cluster_names = ", ".join([c.get("cluster_name", "") for c in item.get("clusters", []) or [] if c.get("cluster_name")])
            cluster_uuids = ", ".join([c.get("cluster_uuid", "") for c in item.get("clusters", []) or [] if c.get("cluster_uuid")])
        ...
        headers = ["Feature Hash", "Type", "Op", "PCode Context", "C Code Context", "Frequency", "TF Score"]
        writer.writerow(headers)
        for item in items:
            ctx = item.get("context", {}) or {}
            c_code_list = ctx.get("c_code") or []
            c_code_str = "".join([t.get("text", "") for t in c_code_list]) if isinstance(c_code_list, list) else ""
            writer.writerow([
                item.get("hash", ""),
                ctx.get("type", ""),
                ctx.get("op", ""),
        ...
        headers = [
            ...
            "Addr 1", "Addr 2",
            "Return Type 1", "Return Type 2", 
            "Namespace 1", "Namespace 2", 
            "Parameters 1", "Parameters 2",
            "Func Tags 1", "Func Tags 2", 
            "Func User Tags 1", "Func User Tags 2",
            "Cluster Names 1", "Cluster UUIDs 1", 
            "Cluster Names 2", "Cluster UUIDs 2", 
            "Feat",
            "File Name 1", "File Name 2", 
            "MD5 1", "MD5 2", 
            "File Tags 1", "File Tags 2",
            "File User Tags 1", "File User Tags 2",
            "Language 1", "Language 2", 
            "Date", 
            "Similarity Tags", "Similarity User Tags"
        ]
        writer.writerow(headers)
        for item in items:
            ...
            cluster_names1 = ", ".join([c.get("cluster_name", "") for c in meta1.get("clusters", []) or [] if c.get("cluster_name")])
            cluster_uuids1 = ", ".join([c.get("cluster_uuid", "") for c in meta1.get("clusters", []) or [] if c.get("cluster_uuid")])
            cluster_names2 = ", ".join([c.get("cluster_name", "") for c in meta2.get("clusters", []) or [] if c.get("cluster_name")])
            cluster_uuids2 = ", ".join([c.get("cluster_uuid", "") for c in meta2.get("clusters", []) or [] if c.get("cluster_uuid")])
        ...
        headers = ["Cluster ID", "UUID", "Name", "Functions", "Stability", "Avg Feat", "Cohesion", "Created"]
        writer.writerow(headers)
        for item in items:
            writer.writerow([
                item.get("cluster_id", ""),
