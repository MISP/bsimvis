...
class TimedPipeline(redis.client.Pipeline):
    ...
    def execute(self, raise_on_error=True):
        ...
        has_lua = any(
            to_str(cmd[0]) in ("EVAL", "EVALSHA") for cmd in self.command_stack
        )
...
def get_redis():
    """Returns the Kvrocks connection for data using a singleton pool."""
    global _kv_pool
    if _kv_pool is None:
...
def get_queue_redis():
    """Returns the standard Redis connection for job queue using a singleton pool."""
    global _redis_pool
