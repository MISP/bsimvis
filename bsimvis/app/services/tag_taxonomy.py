...
def analysis_rules():
    ...
    return (
        ...
        "hang. Reading /proc and comparing process names is process enumeration, not "
        "anti-analysis, anti-VM, or evasion unless visible names identify analysis or "
        "security tools and the control flow changes behavior in response. "
        "Invoking /bin/sh, fork/exec, popen, kill, or a socket API is dual-use by "
        "itself. Assign malicious severity only when the visible function or supplied "
        "call context connects it to untrusted remote commands, destructive target "
        "selection, or another direct operational abuse. "
        "A packet-construction loop that repeatedly sends crafted or randomized packets "
        "to a target is direct traffic-flood behavior even when paced by a sleep; it is "
        "not scanning unless it probes addresses or services to discover targets. "
        "Daemonization, a detached thread, or a long-running loop can establish runtime "
        "residence but is not persistence across reboot; require a visible autostart, "
        "service, cron, bootloader, registry, or equivalent installation mechanism. "
        "For any unresolved recursive-looking or infinite loop, do not label it a "
        "state machine or daemon loop without visible state transitions or daemon setup. "
        "A decoder or compression routine is not evidence of a packer, dropper, "
...
def prompt_rules():
    ...
    return (
        ...
        "operation or resolved callee in the summary. Ordinary user-invoked download clients, servers, archive/crypto tools, and "
        "process inspectors are severity:none when only their normal utility behavior "
        "is visible; assigning a capability category does not raise severity. "
        "Directly implementing operational abuse such as a packet flood, destructive "
        "encryption/wiping, credential theft, or propagation does not require an "
        "identified victim, campaign, operator, C2, or malware family when the abusive "
        "behavior itself is visible. "
        "Repeatedly transmitting crafted "
