...
window.ModuleLoader = {
    ...
    async loadView(viewName, params) {
        ...
        if (module && typeof module.init === 'function') {
            ...
            try {
                await module.init(params, 'module-view-container', signal);
            } catch (err) {
            ...
            setTimeout(async () => {
                ...
                const fetchCount = async (paramKey, paramVal, elId) => {
                    try {
                        ...
                        if (!res.ok || myGen !== _refreshGeneration) return;
                        const data = await res.json();
                        if (data && data.total !== undefined && myGen === _refreshGeneration) {
...
function binSimScoreTypeTagsHtml(p) {
    ...
    const active = isBinCluster ? (p.get('axis') ? (p.get('axis') === 'overall' ? 'score' : `score_${p.get('axis')}`) : `score_${window.BINSIM_DEFAULT_AXIS}`) : (document.getElementById('bsim-score-type')?.value || p.get('sort') || window.BINSIM_DEFAULT_SORT);
...
function updateUI(viewKey, collection, params, route, force = false) {
    ...
    // Always re-render the settings container (view selector pills + limit inputs)
    // so the active pill is correct even when navigating back to the same path.
            ...
            if (path === 'files' || path === 'functions' || path === 'function-similarity' || path === 'features-global') {
                ...
                if (path === 'features-global') {
                    ...
                    headHtml += `
                        ...
                        <th><div class="tag-filter-container" id="tag-container-file"><input type="text" class="tag-filter-add" placeholder="+ Tag" onkeydown="handleTagAdd(event, 'file')" onfocus="attachTagAutocomplete(this, (val) => { createTagCard('file', 'file_tag', val); this.value=''; triggerTagSearch(); })"></div></th>`;
                    ...
                    const extraCells = [
                        ...
                        { html: `<div class="tag-filter-container" id="tag-container-file"><input type="text" class="tag-filter-add" placeholder="+ Tag" onkeydown="handleTagAdd(event, 'file')" onfocus="attachTagAutocomplete(this, (val) => { createTagCard('file', 'file_tag', val); this.value=''; triggerTagSearch(); })"></div>`},
                ...
                headHtml += `<tr class="filter-row">
                    ...
                    <th></th>
                    <th><div class="tag-filter-container" id="tag-container-cluster"><input type="text" class="tag-filter-add" placeholder="+ Tag" onkeydown="handleTagAdd(event, 'cluster')" onfocus="attachTagAutocomplete(this, (val) => { createTagCard('cluster', 'cluster_tag', val); this.value=''; triggerTagSearch(); })"></div></th>
                ...
                headHtml += `<tr class="filter-row">
                    ...
                    <th></th>
                    <th><div class="tag-filter-container" id="tag-container-bin-cluster"><input type="text" class="tag-filter-add" placeholder="+ Tag" onkeydown="handleTagAdd(event, 'bin-cluster')" onfocus="attachTagAutocomplete(this, (val) => { createTagCard('bin-cluster', 'cluster_tag', val); this.value=''; triggerTagSearch(); })"></div></th>
...
function renderBinClusters(items) {
    ...
    return items.map(c => {
        ...
        <tr data-cluster-id="${escapeAttr(c.cluster_id)}"
            ...
            <td class="mono cluster-uuid-id-cell" data-uuid="${escapeAttr(c.cluster_uuid)}" data-id="${escapeAttr(c.cluster_id)}">
                <a href="javascript:void(0)" onclick="event.preventDefault(); openCluster(${escapeAttr(jsString(c.cluster_uuid))}, true, event, ${escapeAttr(jsString(params.get('axis') || window.BINSIM_DEFAULT_AXIS))})" style="color:var(--accent); text-decoration:none;">
                    ${(c.cluster_uuid || '').substring(0, 8)}
                </a>
                <div class="dim" style="font-size:0.7rem">ID: ${c.cluster_id}</div>
            </td>
