                ...
                <tbody>
                    ${(pool.collections || []).map(cname => {
                        ...
                        <tr style="border-bottom: 1px solid var(--border); transition:background 0.2s;" onmouseover="this.style.background='var(--border)'" onmouseout="this.style.background='transparent'">
                            <td style="padding:10px 15px;"><a href="${collUrl}" onclick="Nav.openPath(${escapeAttr(jsString(collUrl))}, event)" style="font-weight:bold; color:var(--text); text-decoration:none;">${escapeHtml(cname)}</a> <button class="btn-action" title="Remove ${escapeAttr(cname)} from pool" onclick="window.poolDetailRemoveCollection(${escapeAttr(jsString(poolId))}, ${escapeAttr(jsString(cname))})"><i class="fa-solid fa-trash-can" style="color:#f87171"></i></button></td>
