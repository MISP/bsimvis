...
def run_index_reg(host, port, args):
    ...
    try:
        ...
        if not keys:
            col_str = (
                f" for '{args.collection}'" if getattr(args, "collection", None) else ""
            )
        ...
        print(
            f"    {'Registry Key':<42} | {'Template':<38} | {'Buckets':<8} | {'Avg':<8} | {'Sample Bucket'}"
        )
        print("-" * 145)
        ...
        for k in sorted(keys):
            try:
                ...
                if (
                    ":reg:tags" in k
                    and ":sim:tags" not in k
                    and ":function:tags" not in k
                ):
                ...
                print(
                    f"    {k:<42} | {template:<38} | {card:<8} | {avg_size:<8.1f} | {sample_bucket}"
                )
            except Exception:
                pass
