...
def run_index_status(host, port, args):
    ...
    num_sim_meta = estimate_total_keys(
        r, f"{coll}:sim_meta:*:*:*", num_files, num_funcs, num_unique_features
    )

    print(f"    - Files           : {num_files}")
    print(
        f"    - Functions       : {num_funcs} ({num_indexed} indexed / {num_funcs - num_indexed} missing)"
    )
    ...
    num_sim_meta_est = estimate_total_keys(
        r, f"{coll}:sim_meta:*:*:*", num_files, num_funcs, num_unique_features
    )
    ...
    stats_config = [
        ...
        (
            "Func Meta",
            f"{coll}:function:*:*:meta",
            num_funcs,
            f"idx:{coll}:all_functions",
            None,
        ),
        (
            "Func Source",
            f"{coll}:function:*:*:source",
            num_funcs,
            f"idx:{coll}:all_functions",
            lambda x: str(x).replace(":meta", ":source"),
        ),
        ...
        (
            ...
            num_unique_features,
            f"idx:{coll}:features:by_tf",
            lambda x: f"idx:{coll}:feature:{x}:functions",
        ),
        (
            "Feature Meta",
            ...
            num_unique_features,
            f"idx:{coll}:features:by_tf",
            lambda x: f"idx:{coll}:feature:{x}:meta",
        ),
    ]
    ...
    for name, pat, count, tset, kform in stats_config:
        ...
        print(
            f"    {name:<20} | {format_bytes(avg):<12} | {format_bytes(comp_total):<12}"
        )

    print("-" * 60)
    print(
        f"    {'ESTIMATED TOTAL INFRA':<20} | {'':<12} | {format_bytes(total_est):<12}"
    )
    ...
    for sec_pattern in scan_patterns:
        ...
        for _ in range(50):
            ...
            for k in keys:
                ...
                if len(parts) >= 5:
                    if parts[2] == "feature":
                        group = (parts[2], parts[4])
                    else:
                        group = (parts[2], parts[3])

                    if group not in sec_indexes:
                        sec_indexes[group] = []
    ...
    print(
        f"    {'Doc':<10} | {'Field':<18} | {'Type':<8} | {'Unique Vals':<12} | {'Avg Docs':<10} | {'Est. Space':<12} | {'Pattern'}"
    )
    print("-" * 140)
    ...
    for (dtype, field), samples in sec_indexes.items():
        ...
        if dtype == "feature" and (field == "functions" or field == "meta"):
            ...
            if len(samples) >= 50:
                ...
                count_acc = 0
                for _ in range(20):
                    inner_cursor, inner_keys = r.scan(
                        inner_cursor, match=f"idx:{coll}:{dtype}:{field}:*", count=2000
                    )
        ...
        if dtype == "sim" and field == "scoreboard":
            val_count_str = f"~{num_funcs}"

        print(
            f"    {dtype:<10} | {field:<18} | {rtype:<8} | {val_count_str:<12} | {avg_docs:<10.1f} | {format_bytes(est_total):<12} | {rep_pat}"
        )
    ...
    print("-" * 60)
    print(
        f"    {'ESTIMATED GRAND TOTAL':<20} | {'':<12} | {format_bytes(total_est + total_sec_est):<12}"
    )

    if num_indexed > 0 and num_unique_features > 0:
        print(f"\n[+] Index Density:")
        print(
            f"    - Unique Features per Function : {num_unique_features / num_indexed:.2f}"
        )
