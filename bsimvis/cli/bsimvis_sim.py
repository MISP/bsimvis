    ...
    if args.action == "status":
        ...
        try:
            ...
            target_str = (
                f"Batch {args.batch}"
                if args.batch
                else (f"MD5 {md5_val}" if md5_val else "Collection")
            )
        ...
        if hasattr(args, "md5") and args.md5:
            ...
            format_str = (
                "{file_md5:<34} | {name:<36} | {total:<8} | {built:<8} | {ratio:>5.1f}%"
            )
        else:
            params = {
                "collection": coll,
        ...
        try:
            ...
            for res in results:
                ...
                if "name" in res:
                    res["name"] = (
                        str(res["name"])[:30]
                        if "batch_uuid" in res
                        else str(res["name"])[:36]
                    )
        ...
        try:
            ...
            print(
                f"[*] Found {data.get('total')} similarities (showing first {len(results)})"
            )
            for res in results:
                print(
                    f"  {res.get('score'):.4f} | {res.get('name1')} <-> {res.get('name2')}"
                )
        except Exception as e:
        ...
        targets = []
        if args.md5:
            targets.extend([{"md5": m} for m in args.md5])
        if args.batch:
