...
def delete_by_prefix(host, port, prefix, batch_size, force):
    ...
    if not force:
        confirm = input(
            f" [!] Are you sure you want to delete all keys starting with '{prefix}'? (y/N): "
        )
        if confirm.lower() != "y":
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="The prefix to match (e.g., 'main:file:')",
    )
    parser.add_argument(
        "--host", type=str, default="localhost", help="Redis host (default: localhost)"
    )
    parser.add_argument(
        "--port", type=int, default=6666, help="Redis port (default: 6666)"
    )
    parser.add_argument(
        "--batch",
        type=int,
        default=1000,
        help="Number of keys to delete per batch (default: 1000)",
    )
