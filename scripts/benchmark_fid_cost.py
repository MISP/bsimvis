...
the analyzer and the per-function matching we added on top of it:
    ...
    comment   + plate-comment parsing only          -> parse     = stream delta
...
MODES = ["off", "analyzer", "comment", "full"]
...
def run_one(gs, orig_extract, target, mode, profile):
    ...
    if mode == "off" or mode == "analyzer":
        patch = lambda self, func, program, q=None, h=None: []
    elif mode == "comment":
        # No query service -> plate-comment parsing only.
        patch = lambda self, func, program, q=None, h=None: orig_extract(
            self, func, program, None, h
        )
    else:
        patch = orig_extract
    ...
    with tempfile.TemporaryDirectory(prefix="bench_fid_") as temp_dir:
        ...
        try:
            ...
            for chunk in gen:
                for f in chunk:
                    funcs += 1
                    # `origin:lib:` since the tag-taxonomy migration; the bare
                    # `lib:` form predates it and is what this counted before.
                    if any(
                        t.startswith(("origin:lib:", "lib:"))
                        for t in f["function_metadata"]["tags"]
                    ):
                        tagged += 1
...
def main():
    ...
    # "comment", not "full": the FID service is opened by stream_bsim_data in
    # every mode, so this warms it without paying the per-function queries.
    run_one(gs, orig_extract, args.targets[0], "comment", args.profile)
